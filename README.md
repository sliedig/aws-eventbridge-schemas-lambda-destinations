...
```bash
ecommerce$ sam local invoke HelloWorldFunction --event events/event.json
```

Don't forget to update the event.json detail with the fields you want to set from your schema.com_aws_orders.ordernotification.OrderNotification object
...
```bash
ecommerce$ sam logs -n HelloWorldFunction --stack-name ecommerce --tail
```
...
```bash
ecommerce$ pip install pytest pytest-mock --user
ecommerce$ python -m pytest tests/ -v
```
